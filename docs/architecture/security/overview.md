...
summary: "这个系统的安全性靠几条彼此正交的隔离边界叠加，没有一道总闸：后端入口的鉴权中间件、对话层的线程隔离、文件层的域隔离、以及执行层的工作区沙箱。理解它们各自挡住什么、各自不管什么，才能知道一条数据从入口到落盘一路被哪些边界约束。"
depends_on:
...
这个系统的安全性靠几条彼此正交的隔离边界叠加，没有一道总闸：后端入口的鉴权中间件、对话层的线程隔离、文件层的域隔离、以及执行层的工作区沙箱。理解它们各自挡住什么、各自不管什么，才能知道一条数据从入口到落盘一路被哪些边界约束。
