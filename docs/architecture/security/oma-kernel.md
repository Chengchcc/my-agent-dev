...
summary: "oma 安全模型的特征是纵深极深、边界有意识留白：路径逃逸防到中间目录 symlink、插件信任防到内容哈希、审批防到超时 deny、SSRF 防到云元数据地址，但 bash 只约束 cwd 不约束命令语义、URL guard 不校验解析后 IP。这些留白是 ADR 0026『single-user local / agents semi-trusted』定位下正确的残余风险接受，不是疏漏；网络化部署会系统性推翻该前提。"
depends_on:
  - ADR 0026
used_by:
  - security.overview
...
> 定位：oma 运行时内核的安全面，包括工具沙箱、URL guard、审批 pipeline、插件信任链、
...
> 所有判断于 2026-09-03 对照代码核实；两处与早先分析的差异已修正（见「修正记录」）。
...
这一判断在 2026-08-26（4633e9af）之前成立，现在已过时。当前内核
