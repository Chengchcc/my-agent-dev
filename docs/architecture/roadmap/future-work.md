...
- **恢复语义的强化**　interruptStore 的 saveInterrupt / consumeInterrupt 已支撑中断恢复。可进一步明确多次中断、反思分叉（`reflect:<threadId>`）与主线恢复之间的交互边界。依赖：[Agent 运行循环](../runtime/framework.md)、[后端总览](../backend/overview.md)。
