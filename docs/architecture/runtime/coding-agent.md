...
status: current
owners: architecture
summary: "coding-agent 是无 UI 的 CLI 执行引擎（print/json/rpc 三种模式），由 Adapter 按 Run spawn 一次性子进程。子进程内 createCodingAgentRuntime() 构造 per-Run Runtime（packages/agent），跑完产出 BackendRunOutcome 后自行退出。"
depends_on:
...
`apps/coding-agent` 是当前唯一的 Agent 执行引擎：一个无 UI 的 CLI，被 Product Backend 的 Adapter 按 **每个 Agent Run** spawn 一次。

```text
Product Backend
  → Adapter (packages/adapter-coding-agent)
      → spawn coding-agent --mode rpc
          → createCodingAgentRuntime()：per-Run Runtime
...
Runtime 内部由 `packages/agent` 提供：CodingAgentSession（模型/工具循环、retry、compaction、beforeModel 插件、todo）与 in-memory SessionStore。SessionStore 在 seed 时把 **full Product history + meta + input** 原子写入，loop 从完整投影开始，Run 结束即销毁 —— 无跨 Run 状态。
...
子进程把 Runtime 事件包装为 `RunEventEnvelope { id, type, data }` 发 stdout；Adapter 用契约包（`@my-agent-team/agent-backend`）的 `mapRunEvent` / `mapRunOutcome` 映射为 `BackendEvent` / `BackendRunOutcome`（completed/failed/aborted/timeout）。outcome 是唯一终态权威。

## 不变量

1. Coding Agent 不是 daemon；每次被 Adapter 按 Run spawn。
