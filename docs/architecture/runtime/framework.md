...
summary: "本页描述旧 Product Runtime 的 in-process 运行循环（AgentHooks/checkpointer/persistence ports）。Phase 5/6 后执行链是 Product Backend → Agent Backend → 一次性 coding-agent 子进程；运行循环位于 packages/agent（Coding Agent Runtime），以 Run 为单位在子进程内执行。本页保留为 tombstone。"
depends_on: []
used_by: []
---
...
→ per-Run Coding Agent Runtime（packages/agent）→ BackendRunOutcome → terminal commit
```
...
相关当前页面：[Coding Agent](./coding-agent.md)、[Agent Backend](../execution/agent-backend.md)。
