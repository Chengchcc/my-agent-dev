...
在 Product Backend 内完成 Agent Run 的创建、排队、执行、恢复和 terminal commit，将 Phase 1 的 Agent Context/Run facts 与 Phase 3 的 Coding Agent Backend 连接起来。交付无 UI 的端到端 Agent Run，但不迁移 Conversation/Cron/Loop 业务入口。

```text
现有 Conversation + Agent Member + Context Branch
→ enqueue/acquire Agent Run
→ dispatch 到 CodingAgentBackend（唯一 Backend，无 registry）
→ Coding Agent 通过 MCP 调用 Product History Tool
→ BackendRunOutcome
→ backend.db 原子提交：
...
- Phase 4 新增模块（agent-run/execution、product-tools）不调用旧 `@my-agent-team/agent`；未切流的 Product caller 留到 Phase 5。
...
- 不创建 fake/in-process Backend；Product Backend 不导入 Provider SDK 或 Coding Agent Runtime。
...
apps/backend/tests/integration/
  agent-run-coding-agent.test.ts
```

可修改：`apps/backend/src/config.ts`、`packages/config/src/env.ts`、`apps/backend/package.json`、`apps/backend/src/bootstrap/features.ts`、`apps/backend/src/infra/db/schema.ts`、`apps/backend/drizzle/backend/0013_*.sql`、`apps/coding-agent/src/worker-runtime.ts`（MCP Bearer token）。

## 实现步骤

1. Backend-only 配置：CODING_AGENT_URL / CODING_AGENT_SERVICE_TOKEN / PRODUCT_TOOLS_MCP_URL / PRODUCT_TOOLS_SERVICE_TOKEN。Provider credential 仍只属于 Coding Agent 服务。
2. 直接构造 CodingAgentClient / CodingAgentBackend / CodingAgentModelCatalog 注入 Execution Service；只支持 backendKind=coding_agent。
...
7. Product Tools MCP：官方 SDK + SSE transport；token 认证；错误归一化为 isError。Coding Agent Worker 的 sse: transport 携带 CODING_AGENT_PRODUCT_TOOL_TOKEN Bearer。
...
- 真实链路：Product Backend → CodingAgentBackend HTTP/SSE → 真实 daemon → 真实 Worker → Product Tools MCP → BackendRunOutcome → backend.db 原子 commit。
- Phase 4 新增模块不 import `@my-agent-team/agent`；Conversation/Cron/Loop/Skill Pack 不调用 Phase 4 服务；全 `apps/backend` 零旧引用属于 Phase 5 clean gate。
