...
class RunnerDaemon {
  ...
  async #drive(runId: string) {
    ...
    try {
      for await (const ev of run.oma.run(run.spec.input, { signal: run.abort.signal, maxSteps: run.spec.maxSteps })) {
  ...
  async #fireReflect(parent: RunHandle) {
    ...
    const reflectAgent = parent.oma.fork({ threadId: `reflect:${parent.spec.threadId}` });
