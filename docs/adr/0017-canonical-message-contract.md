...
### 根因：合法性没有契约保证，全凭各消费方自觉

`Message` 类型允许 `assistant + tool_result`，没有一处强制"模型可回灌"这一不变量。

## 决策

### Canonical 消息契约（协议层，`@chengchenccc/message`）
