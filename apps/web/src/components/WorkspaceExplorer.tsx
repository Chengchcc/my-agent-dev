...
export function WorkspaceExplorer({ agentId }: { agentId: string }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {/* Directory listing */}
      <div className="rounded-lg border border-(--line) bg-(--canvas-soft) p-2 min-h-[320px]">
        <div className="flex items-center gap-2 px-2 py-1.5 text-xs text-(--mute)">
        ...
        <ul className="text-sm">
          {parentPath(path) !== null && (
            <li>
              <button
                type="button"
                className="w-full text-left px-2 py-1 rounded hover:bg-(--canvas) text-(--mute)"
          ...
          {entries.map((e) => (
            <li key={e.name}>
              <button
                type="button"
                className="w-full flex items-center gap-2 px-2 py-1 rounded hover:bg-(--canvas) text-left"
                onClick={() =>
                  e.kind === "dir"
              ...
              >
                {e.kind === "dir" ? (
                  ...
                  <Folder className="size-4 text-(--mute)" />
                ) : (
          ...
          {!loading && entries.length === 0 && (
            <li className="p-2  text-(--mute) text-xs">empty directory</li>
          )}
        </ul>
      </div>
      ...
      {/* File content */}
      {content === null ? (
        <div className="rounded-lg border border-(--line) bg-(--canvas-soft) min-h-[320px] flex flex-col">
          <div className="flex items-center gap-1 px-3 py-2 text-xs text-(--mute) border-b border-(--line)">
            <ChevronRight className="size-3" />
            <span className="font-mono truncate">{selected ?? "select a file"}</span>
          </div>
          <div className="flex-1 flex items-center justify-center text-xs text-(--mute) p-4">
