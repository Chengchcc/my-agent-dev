...
// files (.oma/product-tools.json), not the run input.
...
/** A REAL oma child process (apps/oh-my-agent/src/cli.ts --mode
...
function startDaemon(
  ...
  const command: OmaCommandConfig = {
    ...
    env: {
      OMA_FAKE_PROVIDER: "1",
      OMA_FAKE_TOOL: JSON.stringify(toolScript),
      ...extraEnv,
    },
  };
  return { tmp, backend: new OmaBackend(command) };
}
