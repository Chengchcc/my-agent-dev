 ...
 *  identity + safety live here as md files; workspace files (AGENTS.md /
 *  SOUL.md / USER.md + knowledge index) remain the agent-specific layer
 *  and are appended when present. Explicit run-input systemPrompt (Loop
 *  scopes) still wins wholesale — callers bypass this builder. */
export function buildSystemPrompt(input: { workspacePrompt?: string; cwd: string }): string {
  const now = new Date();
