 ...
 *  uses `CODING_AGENT_BIN ?? "coding-agent"` with no args; tests use the Bun
 *  executable with the app entry source as the arg. */
export interface CodingAgentCommandConfig {
  executable: string;
...
export function spawnCodingAgentProcess(
  cfg: CodingAgentCommandConfig,
  opts: { cwd: string },
): SpawnedCodingAgentProcess {
  ...
  try {
    ...
    throw new ProcessSpawnError(
      `failed to spawn coding-agent executable "${cfg.executable}": ${
...
function collectSecretsFor(cfg: CodingAgentCommandConfig): string[] {
